...
from model.openfold_multimer.primitives import Linear, LayerNorm, ipa_point_weights_init_
...
from model.openfold_multimer.utils.geometry.quat_rigid import QuatRigid
from model.openfold_multimer.utils.geometry.rigid_matrix_vector import Rigid3Array
from model.openfold_multimer.utils.geometry.vector import Vec3Array
from model.openfold_multimer.utils.feats import (
    frames_and_literature_positions_to_atom14_pos,
    torsion_angles_to_frames,
)
from model.openfold_multimer.utils.rigid_utils import Rotation, Rigid
from model.openfold_multimer.utils.tensor_utils import (
    dict_multimap,
    permute_final_dims,
    flatten_final_dims,
)
...
class InvariantPointAttention(nn.Module):
    ...
    def forward(
        ...
        # happens in float32.
        # [*, N_res, H, P_v]
        #print('v_pts: ', v_pts.x.size())
        #print('a: ', permute_final_dims(a, (1, 2, 0)).unsqueeze(-1).size())
        o_pt = v_pts * permute_final_dims(a, (1, 2, 0)).unsqueeze(-1) 
        #print('o_pt: ', o_pt.x.size())
        o_pt = o_pt.sum(dim=-3)
...
class StructureModule(nn.Module):
    ...
    def _forward_multimer(self,
        ...
        ###
        # import numpy as np
        # # tmp = s.detach().cpu().numpy()
        # # np.savez(f'./val_multimer_e2econtact_singlelast_debug/ipa_initial.npz',
        # #              single=tmp)
        # tmp = np.load(f'./val_multimer_e2econtact_singlelast_debug/ipa_initial.npz')
        # tmp = torch.tensor(tmp['single'], dtype=s.dtype).cuda(0)
        # s_initial = tmp
        ###
        s = self.linear_in(s)
        ...
        for i in range(self.no_blocks):
            ...
            # # if i == self.no_blocks - 1:
            # #     import numpy as np
            # #     tmp = np.load('/home/kihara/zhan1797/Desktop/multimer_idp/val_multimer_e2econtact_singlelast_debug/1hr8_O_single_last.npz')
            # #     tmp = torch.tensor(tmp['single'], dtype=s.dtype).cuda(0).unsqueeze(0)
            # #     s = tmp
            # # tmp = s.detach().cpu().numpy()
            # # np.savez(f'./val_multimer_e2econtact_singlelast_debug/ipa_{i}.npz',
            # #          single=tmp)
            # if i > 2:
            #     tmp = np.load(f'./val_multimer_e2econtact_singlelast_debug/ipa_{i}.npz')
            #     tmp = torch.tensor(tmp['single'], dtype=s.dtype).cuda(0)
            #     diff = embedding_loss_single(s, tmp)
            #     print(f'layer {i}: ', torch.mean(diff))
            #     s = tmp
            ###
            ...
            outputs.append(preds)
            outputs_bb.append(rigids.scale_translation(self.trans_scale_factor).to_tensor())
    ...
    def forward(
        ...
        """
        if(self.is_multimer):
            outputs_bb, outputs = self._forward_multimer(s, z, aatype, mask)
        else:
