...
from model.openfold_multimer.utils.geometry import utils
...
class Vec3Array:
    ...
    def __mul__(self, other: Float) -> Vec3Array:
        #for batch
        if not isinstance(other, type(0)) and other.shape[0] > 1:
            batch = other.shape[0]
            x, y, z = list(), list(), list()
            for i in range(batch):
                x.append(self.x[i].unsqueeze(0) * other[i].unsqueeze(0))
                y.append(self.y[i].unsqueeze(0) * other[i].unsqueeze(0))
                z.append(self.z[i].unsqueeze(0) * other[i].unsqueeze(0))
            return Vec3Array(
                torch.cat(x, dim=0),
                torch.cat(y, dim=0),
                torch.cat(z, dim=0),
